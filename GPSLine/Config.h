...
struct Config {
  ...
  bool ENABLE_BMX, ENABLE_MOVING, ENABLE_WATER_GPS, RESPECT_LANE_DIRECTION,
      ENABLE_DISTANCE_TEXT, DISTANCE_UNITS = 0;
...
Config::Config(const char* filename) {
  ...
  /* Navigation */
  RESPECT_LANE_DIRECTION = static_cast<bool>(
      std::atoi(ini["Navigation"]["respectTrafficLaneDirection"].c_str()));
  GPS_LINE_WIDTH =
      static_cast<float>(std::atof(ini["Navigation"]["lineWidth"].c_str()));
  ENABLE_BMX = static_cast<bool>(
      std::atoi(ini["Navigation"]["enableOnBicycles"].c_str()));
  ENABLE_WATER_GPS =
      static_cast<bool>(std::atoi(ini["Navigation"]["enableOnBoats"].c_str()));
  ENABLE_MOVING = static_cast<bool>(
      std::atoi(ini["Navigation"]["trackMovingTargets"].c_str()));
  DISABLE_PROXIMITY =
      static_cast<float>(std::atof(ini["Navigation"]["removeRadius"].c_str()));

  /* Extras */
  ENABLE_DISTANCE_TEXT =
      static_cast<bool>(std::atoi(ini["Extras"]["displayDistance"].c_str()));
  DISTANCE_UNITS =
      static_cast<bool>(std::atoi(ini["Extras"]["distanceUnits"].c_str()));

  /* Custom Colors */
  ENABLE_CUSTOM_CLRS =
      static_cast<bool>(std::atoi(ini["Custom Colors"]["enabled"].c_str()));
  ...
  /* Log */
  LOGFILE_ENABLED =
      static_cast<bool>(std::atoi(ini["Misc"]["enableLog"].c_str()));
